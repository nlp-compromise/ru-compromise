export default [
  'Афганистан', //Afghanistan
  'Албания', //Albania
  'Алжир', //Algeria
  'Американское Самоа', //American Samoa
  'Андорра', //Andorra
  'Ангола', //Angola
  'Ангилья', //Anguilla
  'Антарктика', //Antarctica
  'Антигуа и Барбуда', //Antigua and Barbuda
  'Аргентина', //Argentina
  'Армения', //Armenia
  'Аруба', //Aruba
  'Австралия', //Australia
  'Австрия', //Austria
  'Азербайджан', //Azerbaijan	
  'Багамские Острова', //Bahamas
  'Бахрейн', //Bahrain
  'Бангладеш', //Bangladesh
  'Барбадос', //Barbados
  'Белоруссия', //Belarus
  'Бельгия', //Belgium
  'Белиз', //Belize
  'Бенин', //Benin
  'Бермудские Острова', //Bermuda
  'Бутан', //Bhutan
  'Боливия', //Bolivia
  'Босния и Герцеговина', //Bosnia and Herzegovina
  'Ботсвана', //Botswana
  'Бразилия', //Brazil
  'Бруней', //Brunei
  'Болгария', //Bulgaria
  'Буркина-Фасо', //Burkina Faso
  'Бурунди', //Burundi	'
  'Камбоджа', //Cambodia
  'Камерун', //Cameroon
  'Канада', //Canada
  'Кабо-Верде', //Cape Verde
  'Острова Кайман', //Cayman Islands
  'Центральноафриканская Республика', //Central African Republic
  'Чад', //Chad
  'Чили', //Chile
  'Китай', //China
  'Колумбия', //Colombia
  'Коморские острова', //Comoros
  'Демократическая Республика Конго', //Democratic Republic of the Congo (Kinshasa)
  'Республика Конго', //Congo, Republic of (Brazzaville)
  'Острова Кука', //Cook Islands
  'Коста-Рика', //Costa Rica
  'Кот-д\'Ивуар', //Ivory Coast (Cote d'Ivoire)
  'Хорватия', //Croatia
  'Куба', //Cuba
  'Кипр (остров)', //Cyprus
  'Чехия', //Czechia	'
  'Дания', //Denmark
  'Джибути', //Djibouti
  'Доминика', //Dominica
  'Доминиканская Республика', //Dominican Republic	'
  'Восточный Тимор', //East Timor (Timor-Leste)
  'Эквадор', //Ecuador
  'Египет', //Egypt
  'Сальвадор', //El Salvador
  'Экваториальная Гвинея', //Equatorial Guinea
  'Эритрея', //Eritrea
  'Эстония', //Estonia
  'Эфиопия', //Ethiopia	'
  'Фолклендские острова', //Falkland Islands
  'Фарерские острова', //Faroe Islands
  'Фиджи', //Fiji
  'Финляндия', //Finland
  'Франция', //France
  'Гвиана', //French Guiana
  'Французская Полинезия', //French Polynesia	'
  'Габон', //Gabon
  'Гамбия', //Gambia
  'Грузия', //Georgia
  'Германия', //Germany
  'Гана', //Ghana
  'Гибралтар', //Gibraltar
  'Греция', //Greece
  'Гренландия', //Greenland
  'Гренада', //Grenada
  'Гваделупа', //Guadeloupe
  'Гуам', //Guam
  'Гватемала', //Guatemala
  'Гвинея', //Guinea
  'Гвинея-Бисау', //Guinea-Bissau
  'Гайана', //Guyana	'
  'Гаити', //Haiti
  'Гондурас', //Honduras
  'Гонконг', //Hong Kong
  'Венгрия', //Hungary	'
  'Исландия', //Iceland
  'Индия', //India
  'Индонезия', //Indonesia
  'Иран (Персия)', //Iran (Islamic Republic of)
  'Ирак', //Iraq
  'Ирландия', //Ireland
  'Израиль', //Israel
  'Италия', //Italy	'
  'Ямайка', //Jamaica
  'Япония', //Japan
  'Иордания', //Jordan	'
  'Казахстан', //Kazakhstan
  'Кения', //Kenya
  'Кирибати', //Kiribati
  'Северная Корея', //Korea, Democratic People's Rep. (North Korea)
  'КНДР',
  'Южная Корея', //Korea, Republic of (South Korea)
  'Республика Корея',
  'Косово', //Kosovo
  'Республика Косово',
  'Кувейт', //Kuwait
  'Киргизия', //Kyrgyzstan	'
  'Лаос', //Laos
  'Латвия', //Latvia
  'Ливан', //Lebanon
  'Лесото', //Lesotho
  'Либерия', //Liberia
  'Ливия', //Libya
  'Лихтенштейн', //Liechtenstein
  'Литва', //Lithuania
  'Люксембург', //Luxembourg	'
  'Макао', //Macau
  'Аомынь',
  'Северная Македония', //North Macedonia
  'Мадагаскар', //Madagascar
  'Малави', //Malawi
  'Малайзия', //Malaysia
  'Мальдивы', //Maldives
  'Мали', //Mali
  'Мальта', //Malta
  'Маршалловы Острова', //Marshall Islands
  'Мартиника', //Martinique
  'Мавритания', //Mauritania
  'Маврикий', //Mauritius
  'Майотта', //Mayotte
  'Мексика', //Mexico
  'Федеративные Штаты Микронезии', //Микронезия, Federated States of
  'Молдавия', //Moldova
  'Монако, Княжество Монако', //Monaco
  'Монголия', //Mongolia
  'Черногория', //Montenegro
  'Монтсеррат', //Montserrat
  'Марокко', //Morocco
  'Мозамбик', //Mozambique
  'Мьянма', //Myanmar, Burma
  'Бирма',
  'Намибия', //Namibia
  'Науру', //Nauru
  'Непал', //Nepal
  'Нидерланды, Голландия', //Netherlands
  'Новая Каледония', //New Caledonia
  'Новая Зеландия', //New Zealand
  'Никарагуа', //Nicaragua
  'Нигер', //Niger
  'Нигерия', //Nigeria
  'Северные Марианские', //Northern Mariana Islands
  'Норвегия', //Norway	'
  'Оман', //Oman	'
  'Пакистан', //Pakistan
  'Палау', //Palau
  'Государство Палестина', //State of Palestine
  'Панама', //Panama
  'Папуа — Новая Гвинея', //Papua New Guinea
  'Парагвай', //Paraguay
  'Перу', //Peru
  'Филиппины', //Philippines
  'Польша', //Poland
  'Португалия', //Portugal
  'Пуэрто-Рико', //Puerto Rico	'
  'Катар', //Qatar	'
  'Реюньон', //Réunion
  'Румыния', //Romania
  'Россия', //Russia
  'Российская Федерация', //Russian Federation
  'Руанда', //Rwanda	'
  'Сент-Китс и Невис', //Saint Kitts and Nevis
  'Сент-Люсия', //Saint Lucia
  'Сент-Винсент и Гренадины', //Saint Vincent and the Grenadines
  'Самоа', //Samoa
  'Сан-Марино', //San Marino
  'Сан-Томе и Принсипи', //Sao Tome and Príncipe
  'Саудовская Аравия', //Saudi Arabia
  'Сенегал', //Senegal
  'Сербия', //Serbia
  'Сейшельские Острова', //Seychelles
  'Сьерра-Леоне', //Sierra Leone
  'Сингапур', //Singapore
  'Словакия', //Slovakia (Slovak Republic)
  'Словения', //Slovenia
  'Соломоновы Острова', //Solomon Islands
  'Сомали', //Somalia
  'Южная Африка', //South Africa
  'Южный Судан', //South Sudan
  'Испания', //Spain
  'Шри-Ланка (ранее Цейлон)', //Sri Lanka
  'Судан', //Sudan
  'Суринам', //Suriname
  'Свазиленд', //Swaziland
  'Эсватини', //Eswatini
  'Швеция', //Sweden
  'Швейцария', //Switzerland
  'Сирия', //Syria, Syrian Arab Republic	'
  'Китайская Республика (Тайвань)', //Taiwan (Republic of China)
  'Таджикистан', //Tajikistan
  'Танзания', //Tanzania
  'Таиланд (ранее Сиам)', //Thailand
  'Тибет', //Tibet
  'Восточный Тимор', //Timor-Leste (East Timor)
  'Того', //Togo
  'Токелау', //Tokelau
  'Тонга', //Tonga
  'Тринидад и Тобаго', //Trinidad and Tobago
  'Тунис', //Tunisia
  'Турция', //Turkey
  'Туркмения', //Turkmenistan
  'Тувалу', //Tuvalu	'
  'Уганда', //Uganda
  'Украина', //Ukraine
  'Объединённые Арабские Эмираты (ОАЭ)', //United Arab Emirates
  'Великобритания', //United Kingdom
  'Соединённое Королевство', //United Kingdom
  'Соединённые Штаты Америки', //United States
  'США', //USA
  'Америка', //America
  'Уругвай', //Uruguay
  'Узбекистан', //Uzbekistan	'
  'Вануату', //Vanuatu
  'Ватикан (Святой Престол)', //Vatican City State (Holy See)
  'Венесуэла', //Venezuela
  'Вьетнам', //Vietnam
  'Виргинские Острова (Великобритания)', //Virgin Islands (British)
  'Виргинские Острова', //Virgin Islands (U.S.)	' (США)
  'Уоллис и Футуна', //Wallis and Futuna Islands
  'Западная Сахара', //Western Sahara	'
  'Йемен', //Yemen	'
  'Замбия', //Zambia
  'Зимбабве', //Zimbabwe
]